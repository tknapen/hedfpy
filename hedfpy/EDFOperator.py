...
class EDFOperator(Operator):
    ...
    def __init__(self, input_object, **kwargs):
        ...
        if self.input_object.__class__.__name__ == 'str':
            ...
            self.logger.info(
                'started with '+os.path.split(self.input_file_name)[-1])
    ...
    def read_session_information(self):
        ...
        """
        self.logger.info('reading session information from %s' %
                         self.message_file)
        ...
        self.recording_datetime = datetime.strptime(
            date_string, "%b %d %H:%M:%S %Y")  # %a
    ...
    def identify_blocks(self, minimal_time_gap=50.0, minimal_block_duration=30e3):
        ...
        """
        self.logger.info('identifying recording blocks from %s' %
                         self.gaze_file)
        # check whether the gaze data has been cleaned up
        if not os.path.isfile(self.gaze_file + '.gz'):
            self.logger.error(
                'gaze information not cleaned before running block identification')
        ...
        block_sample_occurrences = re.findall(
            re.compile(sample_re), self.message_string)

        screen_re = 'MSG\t[\d\.]+\tGAZE_COORDS (\d+.\d+) (\d+.\d+) (\d+.\d+) (\d+.\d+)'
        block_screen_occurrences = re.findall(
            re.compile(screen_re), self.message_string)
        ...
        for i, b in enumerate(self.blocks):
            minimal_block_duration_samples = minimal_block_duration * \
                b['sample_rate'] / 1000.0
            self.logger.info('%f raw block duration, threshold %f' % (
                b['block_end_timestamp'] - b['block_start_timestamp'], minimal_block_duration_samples))
        ...
        for bl in self.blocks:
            if bl['eye_recorded'] == 'LR':
                bl.update({'data_columns': ['time', 'L_gaze_x', 'L_gaze_y', 'L_pupil', 'R_gaze_x',
                                            'R_gaze_y', 'R_pupil', 'L_vel_x', 'L_vel_y', 'R_vel_x', 'R_vel_y']})
            elif bl['eye_recorded'] == 'R':
                bl.update({'data_columns': [
                          'time', 'R_gaze_x', 'R_gaze_y', 'R_pupil', 'R_vel_x', 'R_vel_y']})
            elif bl['eye_recorded'] == 'L':
                bl.update({'data_columns': [
                          'time', 'L_gaze_x', 'L_gaze_y', 'L_pupil', 'L_vel_x', 'L_vel_y']})

        self.logger.info('%i correct duration blocks discovered' %
                         len(self.blocks))
    ...
    def read_all_messages(self):
        ...
        # loads the entire message file into an internal variable self.message_string
        self.get_message_string()
        self.read_trials()
    ...
    def read_trials(self,
        ...
        if len(self.start_trial_strings) > 0:
            ...
            self.logger.info(
                'no trial or phase information in edf file %s' % self.input_file_name)
            self.nr_trials = 0
        ...
        self.message_string = self.message_string.replace(
            ' [', '').replace('.]', '')

        parameters = []
        for i in range(self.nr_trials):
            this_re = parameter_re.replace(' X ', ' ' + str(i) + ' ')
            parameter_strings = re.findall(
                re.compile(this_re), self.message_string)
            ...
            try:
                ...
                if nr_double_trials > 1:
                    ...
                    for d in range(nr_double_trials):
                        parameter_strings2.append(
                            parameter_strings[nr_param:nr_param+nr_params])
                        nr_param += nr_params
        ...
        if len(parameters) > 0:        # there were parameters in the edf file
            ...
            ptd = [(k, np.float64) for k in np.unique(
                np.concatenate([list(k.keys()) for k in self.parameters]))]
    ...
    def read_key_events_unix(self,
        ...
        self.logger.info('reading key_events from %s',
        ...
        for i in range(self.nr_trials):
            ...
            event_strings = re.findall(re.compile(
                this_key_re), self.message_string)
            if len(event_strings) > 0:
                if len(event_strings[0]) == 8:
                    events.append([{'EL_timestamp': float(e[0]), 'event_type':int(e[1]), 'up_down':e[2], 'scancode':int(
                        e[3]), 'key':int(e[4]), 'modifier':int(e[6]), 'exp_timestamp':float(e[7])} for e in event_strings])
                    this_length = 8
                elif len(event_strings[0]) == 3:
                    events.append([{'EL_timestamp': float(e[0]), 'event_type':int(
                        e[1]), 'exp_timestamp':float(e[2])} for e in event_strings])
                    this_length = 3
        if len(events) > 0:
            ...
            #
            if this_length == 8:
                self.event_type_dictionary = np.dtype([('EL_timestamp', np.float64), ('event_type', np.float64), ('up_down', '|S25'), (
                    'scancode', np.float64), ('key', np.float64), ('modifier', np.float64), ('exp_timestamp', np.float64)])
            elif this_length == 3:
                self.event_type_dictionary = np.dtype(
                    [('EL_timestamp', np.float64), ('event_type', np.float64), ('exp_timestamp', np.float64)])
    ...
    def read_key_events_unix2(self,
        ...
        for i in range(self.nr_trials):
            ...
            if len(event_strings) > 0:
                if len(event_strings[0]) == 8:
                    ...
                    events.append([{'EL_timestamp': float(
                        e[0]), 'exp_timestamp':float(e[1])} for e in event_strings])
                    this_length = 2
        ...
        if len(events) > 0:
            ...
            if this_length == 8:
                ...
                self.event_type_dictionary = np.dtype(
                    [('EL_timestamp', np.float64), ('exp_timestamp', np.float64)])
        ...
        for i in range(self.nr_trials):
            ...
            if len(event_strings) > 0:
                if len(event_strings[0]) == 7:
                    events.append([{'EL_timestamp': float(e[0]), 'event_type':int(e[1]), 'up_down':e[2], 'scancode':100, 'key':int(
                        e[3]), 'modifier':int(e[5]), 'exp_timestamp':float(e[6])} for e in event_strings])
                    this_length = 8
                elif len(event_strings[0]) == 3:
                    events.append([{'EL_timestamp': float(e[0]), 'event_type':int(
                        e[1]), 'exp_timestamp':float(e[2])} for e in event_strings])
    ...
    def read_eyelink_events(self,
        ...
        """
        self.logger.info('reading eyelink events from %s',
                         os.path.split(self.message_file)[-1])
        self.get_message_string()
        ...
        if len(saccade_strings) > 0:
            ...
            self.saccades_from_message_file = [{'eye': e[0], 'start_timestamp':float(e[1]), 'end_timestamp':float(e[2]), 'duration':float(e[3]), 'start_x':float(
                e[4]), 'start_y':float(e[5]), 'end_x':float(e[6]), 'end_y':float(e[7]), 'length':float(e[8]), 'peak_velocity':float(e[9])} for e in saccade_strings]
            self.fixations_from_message_file = [{'eye': e[0], 'start_timestamp':float(e[1]), 'end_timestamp':float(
                e[2]), 'duration':float(e[3]), 'x':float(e[4]), 'y':float(e[5]), 'pupil_size':float(e[6])} for e in fix_strings]
            self.blinks_from_message_file = [{'eye': e[0], 'start_timestamp':float(
                e[1]), 'end_timestamp':float(e[2]), 'duration':float(e[3])} for e in blink_strings]

            self.saccade_type_dictionary = np.dtype([(s, np.array(
                self.saccades_from_message_file[0][s]).dtype) for s in self.saccades_from_message_file[0].keys()])
            self.fixation_type_dictionary = np.dtype([(s, np.array(
                self.fixations_from_message_file[0][s]).dtype) for s in self.fixations_from_message_file[0].keys()])
            if len(self.blinks_from_message_file) > 0:
                self.blink_type_dictionary = np.dtype([(s, np.array(
                    self.blinks_from_message_file[0][s]).dtype) for s in self.blinks_from_message_file[0].keys()])
            else:
    ...
    def read_sound_events(self,
        ...
        """
        self.logger.info('reading sounds from %s',
                         os.path.split(self.message_file)[-1])
        self.get_message_string()
        if not hasattr(self, 'nr_trials'):
            self.read_trials()
        ...
        sounds.append([{'EL_timestamp': float(s[0]), 'sound_type':int(
            s[1]), 'exp_timestamp':float(s[2])} for s in sound_strings])
        self.sounds = list(chain.from_iterable(sounds))
        #
        # add types to eventTypeDictionary that specify the relevant trial and time in trial for this event - per run.
        #
        self.sound_type_dictionary = np.dtype(
            [('EL_timestamp', np.float64), ('sound_type', np.float64), ('exp_timestamp', np.float64)])
    ...
    def take_gaze_data_for_blocks(self):
        ...
        with open(self.gaze_file) as gfd:
            ...
            for i, block in enumerate(self.blocks):
                ...
                self.logger.info('found data from block %i of shape %s' % (
                    i, str(block['block_data'].shape)))
