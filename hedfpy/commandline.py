...
def ExecCommandLine(cmdline):
    ...
    try:
        retcode = subprocess.call(
            cmdline, shell=True, bufsize=0, stdout=tmpf, stderr=tmpf)
    finally:
        tmpf.close()
        if retcode > 0:
            raise ValueError('Process: '+cmdline +
                             ' returned error code: '+str(retcode))
    return retcode
...
class CommandLineOperator(Operator):
    def __init__(self, input_object, cmd, **kwargs):
        ...
        if self.input_object.__class__.__name__ == 'NiftiImage':
            ...
            self.logger.info(self.__repr__() + ' initialized with ' +
                             os.path.split(self.input_file_name)[-1])
        elif self.input_object.__class__.__name__ == 'str':
            self.input_file_name = self.input_object
            self.logger.info(self.__repr__(
            ) + ' initialized with file ' + os.path.split(self.input_file_name)[-1])
            if not os.path.isfile(self.input_file_name):
                self.logger.warning(
                    'input_file_name is not a file at initialization')
        elif self.input_object.__class__.__name__ == 'list':
            self.inputList = self.input_object
            self.logger.info(self.__repr__() +
                             ' initialized with files ' + str(self.inputList))
        self.cmd = cmd
...
class EDF2ASCOperator(CommandLineOperator):
    ...
    def configure(self, gazeOutputFileName=None, messageOutputFileName=None, settings=' -t -ftime -y -z -v '):
        ...
        if gazeOutputFileName == None:
            self.gazeOutputFileName = os.path.splitext(
                self.input_file_name)[0] + '.gaz'
        else:
            self.gazeOutputFileName = gazeOutputFileName
        if messageOutputFileName == None:
            self.messageOutputFileName = os.path.splitext(
                self.input_file_name)[0] + '.msg'
        else:
            self.messageOutputFileName = messageOutputFileName
        standardOutputFileName = os.path.splitext(
            self.input_file_name)[0] + '.asc'

        self.intermediatecmd = self.cmd
        self.intermediatecmd += settings

        if sys.platform.startswith('win'):
            self.gazcmd = self.intermediatecmd + '-s -miss 0.0001 -vel "{}" & move "{}" "{}"'.format(os.path.normpath(
                self.input_file_name), os.path.normpath(standardOutputFileName), os.path.normpath(self.gazeOutputFileName))
            self.msgcmd = self.intermediatecmd + '-e "{}" & move "{}" "{}"'.format(os.path.normpath(
                self.input_file_name), os.path.normpath(standardOutputFileName), os.path.normpath(self.messageOutputFileName))
            self.runcmd = self.gazcmd + ' && ' + self.msgcmd
        else:
            self.gazcmd = self.intermediatecmd + '-s -miss 0.0001 -vel "'+self.input_file_name+'"; mv ' + '"' + \
                standardOutputFileName.replace(
                    '|', '\|') + '" "' + self.gazeOutputFileName.replace('|', '\|') + '"'
            self.msgcmd = self.intermediatecmd + '-e "'+self.input_file_name+'"; mv ' + '"' + \
                standardOutputFileName.replace(
                    '|', '\|') + '" "' + self.messageOutputFileName.replace('|', '\|') + '"'
            self.runcmd = self.gazcmd + '; ' + self.msgcmd
